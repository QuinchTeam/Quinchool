...
Adapter              src/lib/text-generation/adapters/openai.ts
   ...
   ▼
Provider Service     src/lib/services/openai-text.ts
...
1. Add a service wrapper: `src/lib/services/anthropic-text.ts` (SDK client + call).
2. Add an adapter: `src/lib/text-generation/adapters/anthropic.ts` implementing
...
5. List which models it serves in `providerModels` on the relevant entries in
   `text-generation-models.ts` — the provider chain picks it up automatically,
   no separate list to maintain.

## Adding a new model

Add one entry to `TEXT_GENERATION_MODELS` in `text-generation-models.ts` with
